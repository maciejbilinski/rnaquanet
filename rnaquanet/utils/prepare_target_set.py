...
def read_target_csv(config: RnaquanetConfig, dataset: str):        
    ...
    df[config.data.download.csv_structure_column_name] = df[config.data.download.csv_structure_column_name].apply(lambda description: os.path.join(config.data.path, config.name, 'archive', dataset, description+'.pdb'))
