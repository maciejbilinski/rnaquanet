...
def _rnagrowth_subprocess_func(config: RnaquanetConfig, in_filename:str, child:str):
    """
    Launches RNAgrowth tool and extracts features from PDB file.
    """

    copyfile(os.path.join(config.tools_path,'RNAgrowth','completeAtomNames.dict'), os.path.join(child, 'completeAtomNames.dict'))
    copyfile(os.path.join(config.tools_path, 'RNAgrowth','config.properties'), os.path.join(child, 'config.properties'))
    copyfile(in_filename, os.path.join(child,os.path.basename(in_filename)))

    s = subprocess.Popen([
        'java', '-jar', 
        os.path.join(config.tools_path, 'RNAgrowth','RNAgrowth.jar'), 
        os.path.join(child,os.path.basename(in_filename)), 
        config.data.features.atom_for_distance_calculations, 
        config.data.features.max_euclidean_distance
    ], stdout=subprocess.PIPE, cwd=child)
    s.wait()
...
def extract_features(config: RnaquanetConfig, filtered_file_path: str) -> str:
    """
    ...
    tool_path = os.path.join(config.tools_path, 'RNAgrowth')

    if not os.path.exists(tool_path):
        raise Exception(f"RNAgrowth tools ('{tool_path}') does not exist.")
    ...
    if os.path.exists(features_file_path):
        if not config.data.features.regenerate_features_when_exists:
            return
        else:
