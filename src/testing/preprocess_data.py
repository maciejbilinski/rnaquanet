...
if __name__ == '__main__':
    ...
    # TEST if all files have been filtered (number of files is matching)
    assert len(os.listdir(f'data/{config.data.download.name}/preprocessing/filtered/test')) == 3
    assert len(os.listdir(f'data/{config.data.download.name}/preprocessing/filtered/train')) == 1
    ...
    assert str(data_lists['test']) == """[Data(x=[41, 96], edge_index=[2, 360], edge_attr=[360, 2], y=7.2129998207092285), Data(x=[41, 96], edge_index=[2, 386], edge_attr=[386, 2], y=6.576000213623047), Data(x=[41, 96], edge_index=[2, 378], edge_attr=[378, 2], y=14.145999908447266)]"""
    ...
    assert str(load_data_from_hdf5(config, 'train.h5')) == str(data_lists['train'])
